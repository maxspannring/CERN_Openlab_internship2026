...
if [ -z $1 ]; then
  ...
  koji add-pkg --owner=${KOJICI_USER} cmsoms8el-testing ${pkg}
  echo "Koji build source rpm: $srcrpm"
  koji build --wait cmsoms8el $srcrpm
  #echo "Koji pre-add package to qa: $srcrpm"
  #koji add-pkg --owner=${KOJICI_USER} cmsoms8el-qa ${pkg}
  #koji tag cmsoms8el-qa $rpmname
  echo "Koji pre-add package to production: $srcrpm"
  koji add-pkg --owner=${KOJICI_USER} cmsoms8el-stable ${pkg}
  koji tag --wait cmsoms8el-stable $rpmname
else
