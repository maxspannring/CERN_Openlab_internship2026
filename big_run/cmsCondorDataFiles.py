...
parser.add_option("-q","--flavour",dest="jobFlavour",type="str",default="workday",help="job FLAVOUR") # What does this mean?
...
jobsRoot = f"{MYDIR}/Jobs_{opts.jobTag}"
if os.path.exists(jobsRoot): os.system(f"rm -rf {jobsRoot}")
os.system(f"mkdir -p {jobsRoot}")
...
for i in range(0, nJobs):
    jobDir = f"{jobsRoot}/job_{i}/"
    os.system(f'mkdir {jobDir}')

    with open(f"{jobDir}/sub_{i}.sh", 'w') as tmp_job:
        tmp_job.write("#!/bin/bash\n")
        tmp:job.write("set -euo pipefail # https://gist.github.com/mohanpedala/1e2ff5661761d3abd0385e8223e16425?permalink_comment_id=3799230")
        ...
        tmp_job.write(f"cp output.root {remoteDir}/{opts.jobTag}_{opts.outPrefix}_job{i}.root\n")
...
condor_str += f'+JobFlavour = "{opts.jobFlavour}"\nqueue filename matching ({jobsRoot}/job_*/*.sh)'

subFileName = f"condor_cluster_{opts.jobTag}.sub"
with open(subFileName, "w") as f: f.write(condor_str)
os.system(f"condor_submit {subFileName}")
